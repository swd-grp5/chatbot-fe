...
import { ArrowLeft, Columns2, Search, Upload, Trash2, Pencil, RefreshCw, Eye } from "lucide-react";
...
import type {
  DocumentModalMode,
  DocumentViewMode,
} from "@/features/lecturer/components/document-modal";
...
export function LecturerDocumentsPage() {
  ...
  const selectedSubjectId = subjects.find((subject) => subject.code === selectedCourse)?.id ?? "";
  ...
  const handleToggleActive = async (doc: Doc) => {
    ...
    try {
      ...
      const message =
        e instanceof ApiError ? e.message : e instanceof Error ? e.message : "Cập nhật thất bại";
                          ...
                          <TableRow key={d.id} className={cn(isInactive && "opacity-50")}>
                            ...
                            {columnVisibility.status && (
                              <TableCell className="text-center" style={cell("status")}>
                                <Badge
                                  variant="outline"
                                  className={cn("gap-1.5 font-normal", s.className)}
                                >
                                  {s.label}
                                </Badge>
                              </TableCell>
